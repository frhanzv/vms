...
<body
    ...
    <div class="flex h-screen w-full flex-col">
        <div class="flex flex-1 overflow-hidden">
            ...
            <main
                class="flex-1 overflow-y-auto bg-background-light dark:bg-background-dark custom-scrollbar p-6 lg:p-10">
                <div class="mx-auto max-w-7xl flex flex-col gap-6">
                    ...
                    <div
                        class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
                        <div class="flex items-center gap-2 mb-5">
                            <span class="material-symbols-outlined text-primary">search</span>
                            <h2 class="text-lg font-bold text-slate-900 dark:text-white">Search Visitor</h2>
                        </div>
                        <?php
                        $searchGrid = 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-x-4 gap-y-3';
                        $fieldLabel = 'text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest';
                        $fieldInput = 'w-full border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2.5 text-sm bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all';
                        ?>
                        <div class="space-y-2">
                            <div class="<?= $searchGrid ?>">
                                <label class="<?= $fieldLabel ?>">Search By</label>
                                <label class="<?= $fieldLabel ?>">Search Term</label>
                                <label class="<?= $fieldLabel ?>">From Date/Time</label>
                                <label class="<?= $fieldLabel ?>">To Date/Time</label>
                            ...
                            <div class="<?= $searchGrid ?> items-center">
                                ...
                                <input type="text" id="chronology_to" class="<?= $fieldInput ?> flatpickr-input"
                    ...
                    <div id="chronologySummary"
                        class="hidden flex flex-wrap items-center gap-4 text-sm text-slate-600 dark:text-slate-400">
                    ...
                    <div id="chronologyResultsWrap" class="hidden flex-col gap-4">
                        ...
                        <div
                            ...
                            <div class="flex gap-2">
                                ...
                                <button type="button" onclick="exportVisitorsExcel()"
                                    class="flex items-center gap-2 bg-[#53b2ec] hover:bg-[#46a2db] text-white px-4 py-2 rounded-md text-sm font-medium transition-colors shadow-sm">
                ...
                <div id="chronologyEmpty"
                    class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-16 flex flex-col items-center justify-center gap-3">
                    <span
                        class="material-symbols-outlined text-5xl text-slate-200 dark:text-slate-700">travel_explore</span>
                    <p class="text-slate-500 font-semibold text-sm text-center">No visitor details found</p>
                    <p class="text-slate-400 text-sm text-center max-w-md">Try searching with a different staff number
                        or IC number, widen the date range, or choose “All Lanes”.</p>
                </div>
        </div>
        </main>
    </div>
        ...
        <div
            ...
            <div class="flex items-center justify-between bg-[#00bcd4] px-6 py-3">
                ...
                <button type="button" onclick="closeTimelineModal()"
                    class="text-white border border-white/50 hover:bg-white/10 rounded p-0.5">
                    <span class="material-symbols-outlined text-[20px]">close</span>
                </button>
            </div>
            ...
            <div id="timelineModalContent"
                ...
                <div id="timelineDataContent" class="hidden space-y-6">
                    <!-- Summary Meta -->
                    <div
                        class="border border-slate-100 dark:border-slate-800 rounded-lg p-4 flex flex-col md:flex-row gap-8 justify-center">
                        <div>
                            <span class="text-sm font-bold text-slate-400 block mb-1">Full Name:</span>
                            <span id="tmFullname"
                                class="text-base font-black text-slate-700 dark:text-slate-200 uppercase"></span>
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div class="bg-[#1b7145] rounded-lg p-5 text-white shadow-md">
                            <span class="text-xs font-bold uppercase tracking-wider block opacity-90">Total Time
                                Spent</span>
                        ...
                        <div class="bg-[#ffc107] rounded-lg p-5 text-slate-800 shadow-md">
                            <span class="text-xs font-bold uppercase tracking-wider block opacity-90">Total
                                Visits</span>
                    ...
                    <div class="border border-slate-100 dark:border-slate-800 rounded-lg p-4 bg-slate-50/50">
                        <label
                            class="flex items-center gap-2 text-xs font-black text-slate-700 mb-3 uppercase tracking-tighter">
                    ...
                    <div class="space-y-4">
                        <div class="bg-[#ebf8fa] border border-[#d1f1f5] rounded-lg p-3 flex items-center gap-2">
                            ...
                            <span class="text-xs font-bold text-[#358d99]">Showing data for: <span
                                    id="tmCurrentDate"></span></span>
                        </div>

                        <div class="py-2">
                            <h3
                                class="flex items-center gap-2 text-sm font-black text-slate-700 mb-6 uppercase tracking-tight">
                                <span class="material-symbols-outlined text-[18px]">fork_right</span>
                                Location Movement Timeline
                                <span id="tmDateBadge"
                                    class="bg-primary/20 text-primary text-[10px] px-2 py-0.5 rounded-full ml-1 font-black"></span>
                            </h3>
            ...
            <div
                ...
                <button type="button" onclick="closeTimelineModal()"
                    class="flex items-center gap-2 rounded bg-[#546e7a] hover:bg-[#455a64] px-5 py-2 text-sm font-bold text-white transition-colors">
                    <span class="material-symbols-outlined text-[18px]">close</span>
                    Close
                </button>
            </div>
    ...
    <div id="detailsModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4">
        <div id="detailsModalBackdrop" onclick="closeDetailsModal()"
            class="absolute inset-0 bg-slate-900/55 dark:bg-black/65 cursor-pointer"></div>
        <div
            class="relative flex w-full max-w-2xl flex-col rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900 overflow-hidden">
            <div
                class="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
        ...
        <div class="px-6 py-6 grid grid-cols-2 gap-y-6 gap-x-4 max-h-[70vh] overflow-y-auto custom-scrollbar bg-white dark:bg-slate-900">
            ...
            <div
                ...
                <button type="button" id="btnPrintDetails"
                    class="flex items-center gap-2 rounded bg-primary px-5 py-2 text-sm font-bold text-white shadow-md shadow-primary/20 hover:bg-primary/90 transition-all">
