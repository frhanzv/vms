<!DOCTYPE html>
<?php $lp = $loginPageSettings ?? []; ?>
<html class="light" lang="en">
<head>
    <meta charset="utf-8"/>
    <meta content="width=device-width, initial-scale=1.0" name="viewport"/>
    <title><?= esc($lp['page_title'] ?? 'SafeG - Visitor Management System Login') ?></title>
...
<div class="flex min-h-screen w-full relative">
    ...
    <div class="flex flex-col w-full lg:w-[45%] xl:w-[40%] bg-background-light dark:bg-background-dark h-screen overflow-y-auto relative z-10 shadow-xl">
        <div class="flex flex-col justify-center flex-grow px-8 sm:px-12 md:px-16 lg:px-20 py-12">
            ...
            <!-- Header -->
            <div class="mb-8">
                <h2 class="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white tracking-tight leading-tight mb-3"><?= esc($lp['heading'] ?? 'Welcome back') ?></h2>
                <p class="text-slate-500 dark:text-slate-400 text-base"><?= esc($lp['subheading'] ?? 'Please enter your details to sign in.') ?></p>
            </div>
            ...
            <form action="<?= base_url('auth/attemptLogin') ?>" method="post" class="flex flex-col gap-6 w-full max-w-[480px]">
                ...
                <div class="flex flex-col gap-2">
                    <label class="text-slate-900 dark:text-white text-sm font-semibold leading-normal" for="username"><?= esc($lp['username_label'] ?? 'Username or Email') ?></label>
                    <div class="relative group">
                        ...
                        <input 
                            ...
                            id="username" 
                            name="username"
                            placeholder="<?= esc($lp['username_placeholder'] ?? 'Enter your username or email') ?>" 
                            type="text" 
                            value="<?= old('username') ?>"
                            required
                        />
                ...
                <!-- Password Field -->
                <div class="flex flex-col gap-2">
                    <label class="text-slate-900 dark:text-white text-sm font-semibold leading-normal" for="password"><?= esc($lp['password_label'] ?? 'Password') ?></label>
                    <div class="relative group">
                        ...
                        <input 
                            ...
                            id="password" 
                            name="password"
                            placeholder="<?= esc($lp['password_placeholder'] ?? 'Enter your password') ?>" 
                            type="password"
                            required
                        />
                    </div>
                ...
                <button type="submit" class="flex w-full items-center justify-center rounded-lg bg-primary h-14 px-4 text-base font-bold text-white shadow-md shadow-primary/20 hover:bg-blue-600 hover:shadow-lg hover:shadow-primary/30 active:scale-[0.99] transition-all duration-200 mt-2">
                    <?= esc($lp['login_button_text'] ?? 'Login') ?>
                </button>
            </form>

            <!-- Demo Credentials Info -->
            <div class="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                <p class="text-xs text-blue-800 dark:text-blue-200 font-medium mb-2"><?= esc($lp['demo_title'] ?? 'Demo Credentials:') ?></p>
                <div class="space-y-1">
                    <div>
                        <p class="text-xs text-blue-700 dark:text-blue-300"><?= esc($lp['demo_admin_text'] ?? 'Admin: admin / admin123') ?></p>
                    </div>
                    <div>
                    ...
                    <div>
                        <p class="text-xs text-blue-700 dark:text-blue-300"><?= esc($lp['demo_approver_text'] ?? 'GXO clientsuperadmin: gxoadmin / gxo123') ?></p>
                    </div>
                </div>
            </div>
    ...
    <div class="hidden lg:block lg:w-[55%] xl:w-[60%] relative bg-slate-100 dark:bg-slate-900">
        ...
        <div class="absolute bottom-0 left-0 w-full p-20 flex flex-col justify-end items-start text-white">
            ...
            <h3 class="text-4xl font-bold leading-tight mb-4 max-w-2xl drop-shadow-sm"><?= esc($lp['hero_title'] ?? 'Secure, Seamless Visitor Management.') ?></h3>
            <p class="text-lg text-slate-200 max-w-xl leading-relaxed drop-shadow-md"><?= esc($lp['hero_subtitle'] ?? "Safety Without the Hassle with SafeG's Intelligent Visitor Management System") ?></p>
            <div class="flex items-center gap-4 mt-8 opacity-80">
                ...
                <div class="flex flex-col">
                    <span class="text-sm font-bold"><?= esc($lp['hero_badge_text'] ?? 'A Malaysian Product') ?></span>
