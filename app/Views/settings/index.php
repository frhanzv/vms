        ...
        <div class="flex flex-col gap-8 flex-1 min-h-0">
            ...
            <nav class="flex flex-col gap-2 overflow-y-auto pr-1 custom-scrollbar">
                ...
                <a class="flex items-center gap-3 px-3 py-2.5 rounded-lg <?= $isSettings ? 'bg-primary/10 text-primary' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-primary dark:hover:text-white' ?> transition-colors group" href="<?= base_url('settings') ?>">
                    <span class="material-symbols-outlined text-[22px] <?= $isSettings ? 'font-medium fill-1' : '' ?> group-hover:scale-110 transition-transform">settings</span>
                        ...
                        <div class="pt-2">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" 
                                       ...
                                       value="1" 
                                       <?= ($user['receive_email_notifications'] ?? 1) == 1 ? 'checked' : '' ?>
                                       class="w-4 h-4 text-primary bg-gray-100 border-gray-300 rounded focus:ring-primary dark:focus:ring-primary dark:ring-offset-slate-800 focus:ring-2 dark:bg-slate-700 dark:border-slate-600">
                                <span class="text-sm font-medium text-slate-700 dark:text-slate-300">Receive Email Notifications</span>
                            </label>
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 pl-6">
                                If unchecked, you will not receive event-based emails (e.g. check-in, check-out) even if your role is configured to receive them.
                            </p>
                        </div>
