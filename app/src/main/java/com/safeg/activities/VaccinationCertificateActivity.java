    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!textRecognizer.isOperational()) {
                ...
                public void surfaceCreated(SurfaceHolder holder) {
                    ...
                    try {
                        if (ActivityCompat.checkSelfPermission(getApplicationContext(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                            ...
                            ActivityCompat.requestPermissions(VaccinationCertificateActivity.this,
            ...
            boxDetector.setProcessor(new Detector.Processor<TextBlock>() {
                ...
                public void release() {
                    finish();
                    startActivity(new Intent(VaccinationCertificateActivity.this, CardDetailsActivity.class));
                }
            ...
            public void onFinish() {
                try {
                    if (StaticData.request.ic.isEmpty() || StaticData.request.fullName.isEmpty()) {
                        showMyKadFailedToReadDialog(VaccinationCertificateActivity.this);
                    }
        ...
        findViewById(R.id.rlAddManually).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                finish();
                startActivity(new Intent(getApplicationContext(), VaccinationCertificateDetailsActivity.class));
            }
        });
    }
...
//            int top = (height / 2) - (mBoxWidth / 2);
            ...
            recognizeText(bmp, VaccinationCertificateActivity.this);
                    ...
                    public void onSuccess(Text visionText) {
                        ...
                        if(StaticData.base64_vaccinationCertificate.isEmpty()){
                            if((utils.countLines(visionText.getText()) >= 5
                                    && visionText.getText().toLowerCase().contains("digital certificate")
                                    && visionText.getText().toLowerCase().contains("covid-19 vaccination")
                                    &&
                                    (visionText.getText().toLowerCase().contains("dose 1:")
                                    || visionText.getText().toLowerCase().contains("dose 2:")
                                    || visionText.getText().toLowerCase().contains("booster dose:")))
                            ){

                                try {
                                    if(visionText.getText().toLowerCase().contains("booster dose:")){
                                        List<String> lines = utils.getLines(visionText.getText());
                                        for(int i = 0 ; i < lines.size() ; i++){
                                            if(lines.get(i).toLowerCase().contains("booster dose:")){
                                                StaticData.request.lastDoseDate = lines.get(i+1).replace("Date: ", "");
                                                StaticData.request.lastDoseDate = formatDate("dd-MMM-yyyy hh:mm aaa", "yyyy-MM-dd'T'HH:mm:ss.sssZ", StaticData.request.lastDoseDate);
                                            }
                                        }
                                    } else if(visionText.getText().toLowerCase().contains("dose 2:")){
                                        List<String> lines = utils.getLines(visionText.getText());
                                        for(int i = 0 ; i < lines.size() ; i++){
                                            if(lines.get(i).toLowerCase().contains("dose 2:")){
                                                StaticData.request.lastDoseDate = lines.get(i+1).replace("Date: ", "");
                                                StaticData.request.lastDoseDate = formatDate("dd-MMM-yyyy hh:mm aaa", "yyyy-MM-dd'T'HH:mm:ss.sssZ", StaticData.request.lastDoseDate);
                                            }
                                        }
                                    } else if(visionText.getText().toLowerCase().contains("dose 1:")){
                                        List<String> lines = utils.getLines(visionText.getText());
                                        for(int i = 0 ; i < lines.size() ; i++){
                                            if(lines.get(i).toLowerCase().contains("dose 1:")){
                                                StaticData.request.lastDoseDate = lines.get(i+1).replace("Date: ", "");
                                                StaticData.request.lastDoseDate = formatDate("dd-MMM-yyyy hh:mm aaa", "yyyy-MM-dd'T'HH:mm:ss.sssZ", StaticData.request.lastDoseDate);
                                            }
                                        }
                                    }
                                    StaticData.base64_vaccinationCertificate = new Utils().bitmapToBase64(bitmap);
                                    context.finish();
    ...
    private void showMyKadFailedToReadDialog(AppCompatActivity context) {
        ...
        dialog.setPositiveButtonClick(new Closure() {
            ...
            public void exec() {
                dialog.hide();
                finish();
                context.startActivity(new Intent(context, VaccinationCertificateDetailsActivity.class));
            }
        });
    ...
    String formatDate(String currentFormat, String requiredFormat, String date){
